...
use chris::types::FileBrowserPath;
use chris::{Downloadable, FileBrowser};
...
async fn ls_recursive(
    ...
    while let Some(r) = files_stream.next().await {
        let file = r?;
        let ez_path = coder.decode(file.fname().as_str()).await;
